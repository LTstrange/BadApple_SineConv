...
autoencoder.encoder.requires_grad_(False).cuda()
autoencoder.decoder.requires_grad_(True).cuda()

optimizer = torch.optim.Adam(lr=1e-6, params=autoencoder.parameters())
criterion = nn.MSELoss().cuda()

print("添加训练 training SineConv...")
stime = time.time()
for epoch in range(300):
    ...
    for i, data in enumerate(trainloader, 0):
        ...
        if i % 50 == 49:
            print('[%2d, %5d] loss: %.4f continue training SineConv...' %
